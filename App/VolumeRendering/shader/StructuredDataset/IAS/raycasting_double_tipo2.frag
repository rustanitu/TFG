...
double internal_left_error_evaluated;
double internal_right_error_evaluated;

double external_error_aux;
double external_left_error_evaluated;
double external_right_error_evaluated;
...
bool ColorErrorFunc (dvec4 a, dvec4 b, double tol)
{
  ...
  external_error_aux = max(max(error_r, error_g), max(error_b, error_a));
...
double IterativeExternalIntegration (double s, double h0, double tol)
{
  ...
  {
    h_2 = sqrt ( sqrt (scl_error*maxtol_factor / external_error_aux) ) * h_2;
    h = 2.0*h_2;
    scl_error = ScaleError (tol, h0, h_2);
...
void AdaptiveExternalIntegration (double s, double h, double tol)
{
  ...
  double s1 = s + h;
  lastColor = Cminpost;
  for (int i = 0; i < 10 && s < s1; i++)
  {
    s += IterativeExternalIntegration (s, s1 - s, ScaleError(tol, d, abs(s1 - s)));
  }
  if (abs(s1 - s) > 0) {ForceExternalIntegration (s, s1 - s, ScaleError(tol, d, abs(s1 - s)));}
}

bool AlphaErrorEvalFunc (double a, double b, double tol)
{
  internal_error_aux = abs (b - a);

  return internal_error_aux <= tol;
}
...
double AdaptiveInternalIntegration (double s, double h0, double tol)
{
  ...
  {
    h_2 = sqrt ( sqrt (scl_error*maxtol_factor / internal_error_aux) ) * h_2;
    h = 2.0*h_2;
    scl_error = ScaleError(tol, h0, h_2);
    a = s; b = s + h_2; c = (a + b) / 2.0;
    d = (a + c) / 2.0; e = (c + b) / 2.0;
...
void main()
{
  ...
  while (s < s1)
  {
   ...
   TryExternaIntegration (s, h, ScaleError(tol_error, s1 - s0, h));
   /* if (!TryExternaIntegration (s, h, ScaleError(tol_error, s1 - s0, h) ))
    ...
    double max_tol_h_scaled = ScaleError(tol_error, s1 - s0, h); 
    if ((internal_left_error_evaluated + internal_right_error_evaluated) == 0) h = h == h0 ? 2.0 * h : h;
    else h = sqrt(sqrt(max_tol_h_scaled*maxtol_factor / (internal_left_error_evaluated + internal_right_error_evaluated))) * h;
