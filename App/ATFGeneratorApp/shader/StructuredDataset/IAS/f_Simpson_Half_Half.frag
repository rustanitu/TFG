...
bool TryExternaIntegration (float s, float h, float tol)
{
  ...
  if (ColorErrorFunc (S, S2, tol*15.0f) || h <= 0.04)
  {
...
float Aux_AdaptiveExternalIntegration (float s, float h0, float tol)
{
  ...
  do
  {
    ...
    if (IntegrateExternalInterval (s, s + h_2, scl_error, Fleft[0], Fleft[1], Fleft[2], Fleft[3], Fleft[4], h <= 0.04, aext_left_S))
    {
      colorleft = aux_ext_S;
      break;
    }
  ...
  if (!IntegrateExternalInterval (s + h_2, s + h, scl_error, Fright[0], Fright[1], Fright[2], Fright[3], Fright[4], h <= 0.04, aext_right_S))
  {
    h /= 2.0;
    h_2 = h / 2.0;
...
float AdaptiveInternalIntegration (float s, float h0, float tol)
{
  ...
  do
  {
    ...
    if (IntegrateInternalInterval (s, s + h_2, scl_error, fleft[0].w, fleft[1].w, fleft[2].w, fleft[3].w, fleft[4].w, h <= 0.04, aint_left_S))
    {
      v[0] = aux_int_S;
      v[2] = aux_int_Sleft;
      v[3] = aux_int_Sright;
  ...
  if (!IntegrateInternalInterval (s + h_2, s + h, scl_error, fright[0].w, fright[1].w, fright[2].w, fright[3].w, fright[4].w, h <= 0.04, aint_right_S))
  {
    aint_left_S = v[2];
    aint_right_S = v[3];
...
void main()
{
  ...
  while (s1 > s)
  {
    float h = AdaptiveInternalIntegration (s, h0, ScaleError(tol_int, s1, h0));
    float max_h_tol = ScaleError(tol_ext, s1, h);
