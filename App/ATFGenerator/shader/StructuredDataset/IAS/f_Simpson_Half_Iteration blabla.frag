...
{
  return abs (
  (((h / 12.0) * (anchor_color.w + (4.0 * f_inw.x) + f_inw.y)) + 
  ((h / 12.0) * (f_inw.y + (4.0 * f_inw.z) + f_inw.w))) 
  - 
  ((h / 6.00) * (anchor_color.w + (4.0 * f_inw.y) + f_inw.w)));
}
...
float InternalIntegral (float s, float h0, float error)
{
  ...
  f_inw.x = GetFromTransferFunction (s + (h / 4.0)).w;
  f_inw.y = GetFromTransferFunction (s + (h / 2.0)).w;
  f_inw.z = GetFromTransferFunction (s + (3.0*h / 4.0)).w;
  f_inw.w = GetFromTransferFunction (s + h).w;
...
void main()
{
  ...
  while (s1 > s)
  {
    ...
    if (anchor_color.w + f_inw.x + f_inw.y + f_inw.z + f_inw.w == 0.0)
    {
      anchor_color = GetFromTransferFunction(s + h);
      anchor = s + h;
    }
    else
