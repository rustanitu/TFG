...
void main()
{
  ...
  {   
    ...
    float h_error = ScaleError (tol_error, s1 - s0, h);
    ///////////////////////////
    //h = min (pow (((15.0f*h_error) / IntegrateInternalInterval (s, h, h_error)), 0.25f) * h, 2.0f * h);
    ////h = min (pow (((h_error) / IntegrateInternalInterval (s, h, h_error)), 0.3333f) * h, 2.0f * h);
    //h = min (h, s1 - s);
    //h = min (h, 10.f);
    ///////////////////////////
    hproj = min (0.8f * pow (((15.0f*h_error) / IntegrateInternalInterval (s, h, h_error)), (1.0f / 3.f)) * h, max_step);
    h = min (h, hproj);
    ///////////////////////////
    ...
    {
      ///////////////////////////
      //h = pow (((15.0f*H_error) / MaxVec4Value (external_aux_error)), 0.25f) * h;
      ///////////////////////////
      h = 0.8f * pow (((15.0f*H_error) / MaxVec4Value (external_aux_error)), (1.0f / 3.f)) * h;
      ///////////////////////////
      //h = pow (((H_error) / MaxVec4Value (external_aux_error)), 0.3333f) * h;
