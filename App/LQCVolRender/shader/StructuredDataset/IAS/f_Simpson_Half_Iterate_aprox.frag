...
{
  float x = -((p_d - anchor) / 6.0) * (anchor_color.w + 4.0 * Cmidw + C.w);
  float alphachannel = C.w*pre_integrated*(1.0 + x + (x*x)/2.0 + (x*x*x) / 6.0);
  ...
  while (s1 > anchor)
  {
    ...
    else
    {
      ...
      tol >= abs(S2S.y) && tol >= abs(S2S.z)) || h <= hmin)
      {
        ...
        float x = - h_12 * (anchor_color.w + 4.0 * tf_d.w + 2.0 * tf_c.w + 4.0 * tf_e.w + tf_b.w);
