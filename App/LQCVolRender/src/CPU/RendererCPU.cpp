  ...
  m_camera(rt::RTCamera::CamProjection::PERSPECTIVE)
{
  ...
  m_cam_init_st.eye = glm::vec3(0.0f, 0.0f, number * 1.6f);
  m_cam_init_st.at = glm::vec3(0.0f, 0.0f, 0.0f);
  m_cam_init_st.up = glm::vec3(0.0f, 1.0f, 0.0f);
...
{
  m_aabbmin = glm::vec3(m_aabb_min_x, m_aabb_min_y, m_aabb_min_z);
  m_aabbmax = glm::vec3(m_aabb_max_x, m_aabb_max_y, m_aabb_max_z);
...
glm::vec3 RendererCPU::GetMinAnchor()
{
  return m_aabbmin;
}

glm::vec3 RendererCPU::GetMaxAnchor()
{
  return m_aabbmax;
}

glm::vec4 RendererCPU::Trace(rt::RTRay ray, int  depth, int i, int j)
{
  ...
  if (m_aabb->Hit(ray, tmin, tmax, normal)) {
    ...
    glm::vec3 minpoint = ray.GetPosition() + ray.GetDirection() * tmin;
    glm::vec3 maxpoint = ray.GetPosition() + ray.GetDirection() * tmax;
...
void RendererCPU::RayTracer(int rtwidth, int rtheight)
{
  for (int j = 0; j < rtheight; j++) {
    for (int i = 0; i < rtwidth; i++) {
      glm::vec4 pixel_color = Trace(rt::RTRay(m_camera.GetPositionCastRay((float)i, (float)j)
...
glm::vec4 RendererCPU::Shade(rt::RTObject* object, glm::vec3 minpoint, glm::vec3 maxpoint, glm::vec3 normal, glm::vec3 eye, glm::vec3 eyedirection, int i, int j)
{
  glm::vec4 color;

  if (minpoint != maxpoint) {
    float StepSize = (glm::distance(minpoint, maxpoint) / m_voleval.m_samples);
    glm::vec3 Step = glm::normalize(eyedirection)*StepSize;
    ...
    color = glm::vec4((float)dst.x, (float)dst.y, (float)dst.z, (float)dst.w);
  }
...
void* RendererCPU::RayTracerPThreadCall(void* input)
{
  ...
  for (int j = rt->starter[1]; j < rt->rtheight; j += rt->j_iterator) {
    for (int i = rt->starter[0]; i < rt->rtwidth; i += rt->i_iterator) {
      glm::vec4 pixel_color = rt->cpu_viewer->Trace(rt::RTRay(rt->cpu_viewer->m_camera.GetPositionCastRay((float)i, (float)j)
