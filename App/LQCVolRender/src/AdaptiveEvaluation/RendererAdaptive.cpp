#include "RendererAdaptive.h"

#include <fstream>

RendererAdaptive::RendererAdaptive ()
...
void RendererAdaptive::CreateScene (int CurrentWidth, int CurrentHeight, vr::Volume* volume, gl::GLTexture* tf, bool resetslices)
{
...
bool RendererAdaptive::Render (int Width, int Height)
{
  m_glfbo->Bind ();
...
void RendererAdaptive::Resize (int Width, int Height)
{
  canvas_width = Width;
...
void RendererAdaptive::MouseButtonCb (int b, int p, int x, int y, char* st)
{
  if (p == 1 && b == 49)
  {
    interaction_mode = 0;
...
void RendererAdaptive::MouseMotionCb (int x, int y, char *status)
{
...
void RendererAdaptive::ReloadVolume (vr::Volume* volume)
{
  ...
  if (m_glsl_volume)
  {
    ...
    shader_spass->SetUniformVector3f ("tex_scale", glm::vec3 (
...
void RendererAdaptive::ReloadTransferFunction (vr::TransferFunction* tfunction)
{
...
void RendererAdaptive::ResetModelMatrix ()
{
  model = glm::mat4 ();
}
...
void RendererAdaptive::SetRealTolerance (float tol)
{
  shader_spass->Bind ();
  shader_spass->SetUniformFloat ("tol_error", 15.0f * tol);
  shader_spass->BindUniform ("tol_error");
  shader_spass->Unbind ();
}

void RendererAdaptive::DrawFirstPass ()
{
  shader_fpass->Bind();
...
void RendererAdaptive::DrawSecondPass ()
{
  shader_spass->Bind();
...
void RendererAdaptive::CreateFirstPass ()
{
...
void RendererAdaptive::CreateSecondPass ()
{
  if (shader_id == 0)
    shader_spass = new gl::GLShader ("shader/StructuredDataset/raycasting.vert",
    "shader/StructuredDataset/AdaptiveEvaluation/coupled.frag");
  else if (shader_id == 1)
    shader_spass = new gl::GLShader ("shader/StructuredDataset/raycasting.vert",
    "shader/StructuredDataset/AdaptiveEvaluation/decoupled.frag");
  ...
  shader_spass->SetUniformFloat ("initial_step", .5f);
  shader_spass->SetUniformFloat ("tol_error", 15.0f * 0.01f);
...
void RendererAdaptive::CreateBuffers ()
{
  ...
  const RendererAdaptive::Vertex VERTICES[8] =
  {
...
void RendererAdaptive::DestroyFirstPass ()
{
  if (shader_fpass)
    delete shader_fpass;
  shader_fpass = NULL;
...
void RendererAdaptive::DestroySecondPass ()
{
  if (shader_spass)
    delete shader_spass;
  shader_spass = NULL;
...
void RendererAdaptive::DestroyBuffers ()
{
  if (vbo)
    delete vbo;
  vbo = NULL;
