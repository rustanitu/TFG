...
namespace vr
{
    ...
    : ScalarField(width, height, depth)
  {
	...
	glm::vec3 Volume::GetAnchorMin()
	{
		return m_pmin;
	}

	glm::vec3 Volume::GetAnchorMax()
	{
		return m_pmax;
	}

	void Volume::SetAnchors(glm::vec3 pmin, glm::vec3 pmax)
	{
		m_pmin = pmin;
		m_pmax = pmax;
	}
  ...
  double Volume::CalculateLaplacian(const UINT32& x, const UINT32& y, const UINT32& z)
  {
	...
	double Volume::InterpolatedValue(glm::vec3 pos)
	{
		return InterpolatedValue(pos.x, pos.y, pos.z);
	}

	double Volume::TrilinearScalarFunction(glm::vec3 pos, glm::vec3 rayeye, glm::vec3 raydirection)
	{
